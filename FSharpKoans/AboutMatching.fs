...
module ``04: Match expressions`` = 
    ...
    let ``02 Match expressions are expressions, not statements`` () =
        let result =
            match 9001 with
            | n -> // <-- use an identifier pattern here!
                match n + 1000 with // <-- now use the identifier that you've bound
    ...
    let ``05 Using a mapping function`` () =
        let mapper = function
            | 3 -> "Joey"
            | 8 -> "Bingo"
            | n -> "Kelvin" // write the cases for this function!
