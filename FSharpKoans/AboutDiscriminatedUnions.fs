...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BA _-> "A thinker, eh?"
        randomOpinion (BSc(Sociology, ComputerScience)) |> should equal "Good choice!"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("One", 
                    Node("Two", 
                        Node("Three",
                            Node("Four", Empty, Empty), Empty), Empty), Empty) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
