...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun three -> fun nine -> three * nine) 3 9 |> should equal 27
        // fun three ->(fun nine ->(three * nine)) 3 9

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun ten seven -> ten + seven) 10 7 |> should equal 17
...
*)
    ...
    *)
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows noise =
            f "cow" noise + ", de gozaru" // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
