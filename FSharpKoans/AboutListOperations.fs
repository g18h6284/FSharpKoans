    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec listLen xs i = 
                match xs with
                | [] -> i // base case = empty list
                | a::rest -> listLen rest (i+1) // recursive cases
                
            listLen xs 0// write a function to find the length of a list
            //[9;8;7] 0
            //[8;7] 1
            //[7] 2
            //[] 3
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec revList xs rev =
                match xs with 
                | [] -> rev
                | a::rest -> revList rest (a::rev) // write a function to reverse a list here.
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec mapListAddOne xs m =
                match xs with
                | [] -> List.rev m
                | a::rest ->  mapListAddOne rest (a+1::m)// write a function which adds 1 to each element
            mapListAddOne xs []
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec mapListX2 xs m =
                match xs with
                | [] -> List.rev m
                | a::rest ->  mapListX2 rest (a*2::m)
            mapListX2 xs [] // write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapListFun xs m =
                match xs with
                | [] -> List.rev m
                | a::rest ->  mapListFun rest ((f a)::m)
            mapListFun xs [] // write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec mapList xs m =
                match xs with
                | [] -> List.rev m
                | a::rest -> match f a with
                             | true -> mapList rest (a::m)
                             | false -> mapList rest m
            mapList xs []// write a function which filters based on the specified criteria
        ...
        let filter (xs : int list) : int list =
            let rec mapList xs m =
                ...
                | a::rest ->  match (a%2 <> 0) with
                              | true -> mapList rest (a::m)
                              | false -> mapList rest m
            mapList xs [] // write a function to filter for odd elements only.
...
*)
    ...
    let ``12 A fold which sums a list`` () =
        let rec fold initialState xs =
            match xs with 
            | [] -> initialState
            | a::rest -> fold (initialState+a) rest
    ...
    let ``13 A fold which multiplies a list`` () =
        let rec fold initialState xs =
            match xs with 
            | [] -> initialState
            | a::rest -> fold (initialState*a) rest // write a function to multiply the elements of a list
    ...
    let ``14 Folding, the hard way`` () =
        let rec fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            match xs with 
            | [] -> initialState
            | a::rest -> fold f (f initialState a) rest   // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``16 exists: finding whether any matching item exists`` () =
        let rec exists (f : 'a -> bool) (xs : 'a list) : bool =
            ...
            | a::rest -> match f a with 
                         | true -> true
                         | false -> exists f rest // Does this: https://msdn.microsoft.com/visualfsharpdocs/conceptual/list.exists%5b%27t%5d-function-%5bfsharp%5d
    ...
    let ``17 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec partition f xs a b =
                match xs with
                | [] -> List.rev a, List.rev b
                | c::rest -> match (f c) with 
                             | true -> partition f rest (c::a) b
                             | false -> partition f rest a (c::b)
            partition f xs [] []
    ...
    let ``18 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec init n f x l =
                match n with 
                | 0 -> List.rev l
                | n -> init (n-1) f (x+1) ((f x)::l)
                
            init n f 0 []// Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.init%5b't%5d-function-%5bfsharp%5d
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec tryFind p xs = 
                ...
                | a::rest -> match (p a) with 
                             | true -> (Some a)
                             | false -> tryFind p rest 
                             
            tryFind p xs// Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.tryfind%5b't%5d-function-%5bfsharp%5d
    ...
    let ``20 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            ...
            tryPick p xs // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.trypick%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``21 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec choose xs l= 
                ...
                | a::rest -> match (p a) with
                             | None -> choose rest l
                             | (Some x) -> choose rest (x::l)
                             
            choose xs []// Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.choose%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``22 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapi f xs i l =
                match xs with
                | [] -> List.rev l
                | a::rest -> mapi f rest (i+1) ((f i a)::l)
            mapi f xs 0 []
